...
if __name__ == "__main__":
    ...
    ap.add_argument("-sd", "--square_dimension", required=False, default="0.08", help="square dimension in meters")
    ap.add_argument("-p", "--path", required=False, default="images/", help="path to images folder")
    ap.add_argument("-e", "--file_extension", required=False, default=".png", help="extension of images")
	...
	ret, corners = cv2.findChessboardCorners(gray, chessboardDimension,cv2.CALIB_CB_ADAPTIVE_THRESH+cv2.CALIB_CB_NORMALIZE_IMAGE)
	#print ret	
	if ret:
		if not auto_mode:
			imgCopy = img.copy()
			...
			filename= "output/weld_"+str(datetime.datetime.now().date()) + '_' + str(datetime.datetime.now().time()).replace(':', '.')+".png"
			...
			while True:
		          ...
		          objPoints.append(objp)
		          corners2 = cv2.cornerSubPix(gray, corners, (3, 3), None, criteria)
		          imgPoints.append(corners2)
